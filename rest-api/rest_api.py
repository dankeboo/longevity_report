...
data = f"""search=search index=_introspection  sourcetype=splunk_resource_usage component=Hostwide host=i* 
    | rename data.* as * |eval cpu=cpu_system_pct%2Bcpu_user_pct | bucket span={bucket_span} _time
    ...
    | rename data.* as * |eval cpu=cpu_system_pct %2B cpu_user_pct| bucket span={bucket_span} _time
